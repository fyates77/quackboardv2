...
interface DashboardState {
  ...
  /** Global component library — persisted across dashboards */
  componentLibrary: Panel[];
  ...
  // Page width
  updatePageWidth: (dashboardId: string, width: number) => void;

  // Component library
  saveComponent: (panel: Panel) => void;
  deleteComponent: (id: string) => void;
  insertComponentToDashboard: (componentId: string, dashboardId: string) => void;
  ...
  temporal(
    persist(
      (set) => ({
        ...
        updatePageWidth: (dashboardId, width) =>
          set((state) => {
            const dash = state.dashboards[dashboardId];
            if (!dash) return state;
            return {
        ...
        saveComponent: (panel) =>
          set((state) => {
            ...
            const exists = state.componentLibrary.some((c) => c.id === panel.id);
            return {
              componentLibrary: exists
                ? state.componentLibrary.map((c) => (c.id === panel.id ? panel : c))
                : [...state.componentLibrary, panel],
            };
          }),

        deleteComponent: (id) =>
          set((state) => ({
            componentLibrary: state.componentLibrary.filter((c) => c.id !== id),
          })),

        insertComponentToDashboard: (componentId, dashboardId) => {
          const newId = createId();
          set((state) => {
            const component = state.componentLibrary.find((c) => c.id === componentId);
            const dash = state.dashboards[dashboardId];
            if (!component || !dash) return state;
            const newPanel: Panel = { ...component, id: newId, title: `${component.title} (copy)` };
            const layoutItem: LayoutItem = { i: newId, x: 0, y: Infinity, w: 6, h: 4, minW: 2, minH: 2 };
            return {
              dashboards: {
                ...state.dashboards,
                [dashboardId]: {
    ...
    {
      limit: 50,
      // Only track dashboard data, not UI-only state
